...
import PropertyDetailsScreen from './business/Listyourproperty/screens/PropertyDetailsScreen';
...
const HotelsPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log('Current filters:', { searchTerm, priceRange, rating, selectedPropertyType, sortBy });
    
    // TEMPORARILY DISABLE FILTERING TO DEBUG
    console.log('TEMPORARILY DISABLING FILTERS - showing all properties');
    setFilteredProperties(properties);
    setCurrentPage(1);
    return;
    
    /* COMMENTED OUT FILTERING LOGIC FOR DEBUGGING
    // Restore filtering logic
    const filtered = properties.filter(property => {
      const propertyName = property.propertyName || '';
      const locationStr = typeof property.location === 'string' 
        ? property.location 
        : `${property.location?.city ?? ''} ${property.location?.country ?? ''}`.trim();
      ...
      const matchesRating = rating === 0 || propertyRating >= rating;
      const matchesType = selectedPropertyType === 'all' || roomType === selectedPropertyType;
      
      return matchesSearch && matchesPrice && matchesRating && matchesType;
    });

    console.log('Filtered properties count:', filtered.length); // Debug log
    ...
    switch (sortBy) {
      ...
      case 'distance':
        sorted.sort((a, b) => {
          const aDist = parseInt(a.distanceToCenter?.split(' ')[0] || '0');
          const bDist = parseInt(b.distanceToCenter?.split(' ')[0] || '0');
          return aDist - bDist;
        });
        break;
      default:
